<div class="justify-center w-full mx-auto bg-gray-100 bg-top-nav">
  <div x-data="{ open: false }" class="flex flex-col w-full px-8 py-2 mx-auto md:px-12 md:items-center md:justify-between md:flex-row lg:px-12 max-w-7xl">
    ...
    <nav :class="{'flex': open, 'hidden': !open}" class="flex-col items-center flex-grow hidden gap-3 p-4 px-5 text-sm font-medium text-gray-300 md:px-0 md:pb-0 md:flex md:justify-end md:flex-row lg:p-0 md:mt-0">
      <form method="POST" action="{{ route('logout') }}" x-data class="ml-auto">
        @csrf
        <button dusk= "logout" class="Btn-logout">
          <div class="sign">
